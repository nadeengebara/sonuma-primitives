...
static inline void rmc_check_cq(rmc_wq_t *wq, rmc_cq_t *cq, async_handler *handler, void *owner) {
    ...
    do { // in the upper loop we wait for a free entry in the WQ head
        ...
        while (cq->q[cq_tail].SR == cq->SR) {
            ...
            PASS2FLEXUS_CONFIG(tid, WQENTRYDONE, 0);
#endif
            cq_tail = cq->tail;
...
#elif defined(STATS_BREAKPOINTS)
    PASS2FLEXUS_CONFIG(wq_head, NEWWQENTRY, 0);
#endif /* DEBUG_FLEXUS_STATS or STATS_BREAKPOINTS */
...
#elif defined(STATS_BREAKPOINTS)
    PASS2FLEXUS_CONFIG(cq_tail, WQENTRYDONE, 0);
#endif
    cq->tail = cq->tail + 1;
...
static uint8_t rmc_sabre_sync(rmc_wq_t *wq, rmc_cq_t *cq, uint64_t lbuff_slot, int snid, uint32_t ctx_id, uint64_t ctx_offset, uint64_t length) {
    ...
    if (cq->q[cq_tail].success) {	//No atomicity violation!
        PASS2FLEXUS_CONFIG(cq_tail, SABRE_SUCCESS, 0);
    } else {	//Atomicity violation detected
        PASS2FLEXUS_CONFIG(cq_tail, SABRE_ABORT, 0);
    }
